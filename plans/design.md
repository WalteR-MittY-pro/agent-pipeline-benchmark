...
输出（写入 state）：
  state["prs"]  list[PRMetadata]  — 通过所有过滤条件的 PR 列表（append）
...
| `--input` | str | `prs_snapshot.json` | `build` 模式读取的 PR 文件路径 |
| `--output` | str | `prs_snapshot.json` | `fetch` 模式输出的 PR 文件路径 |
...
### 11.2 `infer_env` → `build_dockerfile` 契约
